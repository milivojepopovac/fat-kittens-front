  ...
  const claimNFTs = async () => {
    ...
    blockchain.smartContract.methods
      ...
      .then((receipt) => {
        console.log(receipt);
        setFeedback(
          `WOW, the ${CONFIG.NFT_NAME} is yours! go visit Sparkles to view it.`
        );
        setClaimingNft(false);
            ...
            {Number(data.totalSupply) >= CONFIG.MAX_SUPPLY ? (
              ...
              <>
                <s.TextTitle
                  style={{ textAlign: "center", color: "var(--accent-text)" }}
                >
                  1 {CONFIG.SYMBOL} costs {data.saleFlag == 1 ?  Web3.utils.fromWei(data.presaleCost, 'ether'): Web3.utils.fromWei(data.cost, 'ether')}{" "}
